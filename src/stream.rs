...
pub struct MessageStart {
    ...
    /// asked for: a gateway routing `gateway/claude-opus-5` reports
    /// `claude-opus-5` here.
    pub model: String,
    /// The opening usage.
    ///
...
pub enum StreamEvent {
    ...
    ///
    /// Carries a [`Usage`], because gateways do: the inference endpoint
...
mod tests {
    ...
    fn a_captured_message_start_decodes() {
        let event = StreamEvent::decode(
            r#"{"type": "message_start", "message": {"model": "claude-opus-5",
                "id": "msg_762xvdbzixaxf3ot2soefimzo5seq5vb7327nuan2pjk62be6rna", "type": "message",
        ...
        assert_eq!(start.id, "msg_762xvdbzixaxf3ot2soefimzo5seq5vb7327nuan2pjk62be6rna");
    ...
    fn a_captured_cache_hit_is_visible_from_the_first_frame() {
        let event = StreamEvent::decode(
            r#"{"type": "message_start", "message": {"model": "claude-opus-5", "id": "msg_x", "type": "message",
    ...
    fn a_captured_tool_use_block_and_its_fragment_decode() {
        let start = StreamEvent::decode(
            r#"{"type": "content_block_start", "index": 0, "content_block": {"type": "tool_use",
                "id": "toolu_01SfSun6GdgxKVDToTxtf7Ee", "name": "get_weather", "input": {}}}"#,
        )
        .unwrap();
        ...
        assert_eq!(id, "toolu_01SfSun6GdgxKVDToTxtf7Ee");
